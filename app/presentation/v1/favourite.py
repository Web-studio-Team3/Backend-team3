...
async def delete(
    ...
    get_access_token_by_jwt_use_case: GetAccessTokenByJwtUseCase = 
    Depends(provide_get_access_token_by_jwt_stub),
    get_favourites_by_user_id: GetFavouritesByUserIdUseCase =
    ...
    if (not any(
        favourite.user_id == user_id for favourite in favourites
    )):
        return {
...
async def get_count_of_favourites_by_item(
    item_id: str,
    get_favourites_count_by_item_id_use_case: GetFavouritesCountByItemIdUseCase =
    Depends(provide_get_favourites_count_by_item_id_stub)
):
