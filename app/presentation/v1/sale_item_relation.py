...
from app.core.sale_item.entities.sale_item_relation import (
    SaleItemRelation as SaleItemRelationDataClass,
)
...
async def create(
    item_id: str,
    create_sale_item_relation_use_case: CreateSaleItemRelationUseCase = Depends(
        provide_create_sale_item_relation_stub
    ),
    ...
    return {"id": sale_item_relation_id.id}


@router.delete(path="/{sale_item_relation_id}")
async def delete(
    sale_item_relation_id: str,
    delete_sale_item_relation_use_case: DeleteSaleItemRelationUseCase = Depends(
        provide_delete_sale_item_relation_stub
    ),
    jwt: str = Depends(JWTBearer()),
    get_access_token_by_jwt_use_case: GetAccessTokenByJwtUseCase = Depends(
    ...
    get_sale_item_realtion_by_id_use_case: GetSaleItemRelationByIdUseCase = Depends(
        provide_get_sale_item_relation_by_id_stub
    ),
):
    try:
        ...
        raise HTTPException(status_code=400, detail="No such relation")
...
async def get(
    sale_item_relation_id: str,
    get_sale_item_relation_by_id_use_case: GetSaleItemRelationByIdUseCase = Depends(
        provide_get_sale_item_relation_by_id_stub
    ),
):
    try:
...
async def get_by_item_id(
    item_id: str,
    get_sale_item_relation_by_item_id_use_case: GetSaleItemRelationByItemIdUseCase = Depends(
        provide_get_sale_item_relation_by_item_id_stub
    ),
):
    try:
...
async def get_by_user_id(
    user_id: str,
    get_sale_item_relation_by_user_id_use_case: GetSaleItemRelationByUserIdUseCase = Depends(
        provide_get_sale_item_relation_by_user_id_stub
    ),
):
