...
from app.core.category.dto.category import (
    CategoryId, CategoryCreate, CategoryUpdate, CategoryUpdateWithId
)
...
async def get_category_all(
    get_category_all_use_case: GetCategoryAllUseCase = Depends(provide_get_categories_stub)
):
    try:
...
async def get_category_by_id(
    category_id: str,
    get_category_by_id_use_case: GetCategoryByIdUseCase = Depends(provide_get_category_by_id_stub),
):
    try:
        ...
        return HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No category with such id"
        )
    return {
        "id": category.id,
...
async def create_category(
    ...
    try:
        category_id = create_category_use_case.execute(category=CategoryCreate(
            title=title,
            childs=childs
        )).id
...
async def update_category(
    ...
    update_category_use_case: UpdateCategoryUseCase = Depends(provide_update_category_stub)
):
    updated_category = update_category_use_case.execute(CategoryUpdateWithId(
        id=category_id,
