...
async def create(
    ...
    if (not any(
        sale_item_relation.item_id == sold_item_relation.item_id for sale_item_relation in sale_item_relations
    )):
        return {
...
async def delete(
    ...
    if (sold_item_relation.seller_id != get_access_token_by_jwt_use_case.execute(jwt).user_id):
        return {
            "message": "Not allowed for this user"
        }
...
async def get(
    ...
    user_id = get_access_token_by_jwt_use_case.execute(jwt).user_id
    if (sold_item_relation.seller_id != user_id and sold_item_relation.buyer_id != user_id):
        return {
            "message": "Not allowed for this user"
        }
