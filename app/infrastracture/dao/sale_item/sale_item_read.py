...
class SaleItemRelationReadImpl(BaseDao, SaleItemRelationRead):
    def get_sale_item_relation_by_id(self, id: str) -> SaleItemRelation:
        relation = self._database["sale_item_relation"].find_one({"_id": ObjectId(id)})
        if not relation:
            raise TypeError
    ...
    def get_sale_item_relation_by_item_id(self, item_id: str) -> SaleItemRelation:
        relation = self._database["sale_item_relation"].find_one({"item_id": item_id})
        if not relation:
            raise TypeError
    ...
    def get_sale_item_relation_by_user_id(self, user_id: str) -> list[SaleItemRelation]:
        relations = self._database["sale_item_relation"].find({"user_id": user_id})
