...
class SoldItemRelationReadImpl(BaseDao, SoldItemRelationRead):
    def get_sold_item_relation_by_id(self, id: str) -> SoldItemRelation:
        relation = self._database["sold_item_relation"].find_one({"_id": ObjectId(id)})
        if not relation:
            raise TypeError
    ...
    def get_sold_item_relation_by_buyer_id(
        ...
        relations = self._database["sold_item_relation"].find({"buyer_id": buyer_id})
        if not relations:
            raise TypeError
    ...
    def get_sold_item_relation_by_item_id(self, item_id: str) -> SoldItemRelation:
        relation = self._database["sold_item_relation"].find_one({"item_id": item_id})
        if not relation:
            raise TypeError
        return create_sold_item_relation(relation)
    ...
    def get_sold_item_relation_by_seller_id(
        ...
        relations = self._database["sold_item_relation"].find({"seller_id": seller_id})
        if not relations:
            raise TypeError
        return list(map(create_sold_item_relation, relations))
