...
class ReviewReadImpl(BaseDao, ReviewRead):
    def get_reviews_by_item_id(self, item_id: str) -> list[Review]:
        ...
        user_dict = {str(user["_id"]): user["first_name"] for user in users}

        return [self.create_review(review, user_dict.get(review["user_id"], "Unknown")) for review in reviews]


def create_review(self, review_db_object, full_name):
    return Review(
