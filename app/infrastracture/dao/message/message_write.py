from bson import ObjectId

from app.core.chat_message.dao.message_write import MessageWrite
from app.core.chat_message.dto.message import Message, MessagesId
from app.infrastracture.dao.base import BaseDao
from app.infrastracture.models.message import MessageModel


class MessageWriteDaoImpl(BaseDao, MessageWrite):
    def add_message(self, message: Message, messages_id: str) -> None:
        messages = (
            self._database["message"]
            .insert_one(
                MessageModel(
                    message_id=message.message_id,
                    user_name=message.user_name,
                    date_time=message.date_time,
                    message=message.message,
                ).dict(exclude_none=True)
            )
            .inserted_id
        )

    def delete_all(self, messages_id: MessagesId) -> None:
        self._database[f"{messages_id}"].drop()

    def delete(self, messages_id: MessagesId, message: Message) -> None:
        self._database[f"{messages_id}"].find_one_and_delete(
            {"_id": ObjectId(message.id)}
        )
