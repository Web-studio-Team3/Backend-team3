...
def categoryRelationEntity(item) -> dict:
    return {
        "id": str(item["_id"]),
        "parent_category": item["parent_category"] if "parent_category" in item.keys() else None ,
        "child_category": item["child_category"] if "child_category" in item.keys() else None ,
    }
