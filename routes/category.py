...
from schemas.category import categoriesEntity, categoryEntity, categoryTreeEntity
...
def find_all_categories():
    ...
    for category in allCategories:
        ...
        if (
            category != None
            and "childs" in category.keys()
            and len(category["childs"]) > 0
        ):
            # print(category)
            allCategories[allCategories.index(category)] = None
            childCategories, allCategories = next_lvl_category(
                category["childs"], allCategories
            )
...
def next_lvl_category(categories, allCategories):
    ...
    for category in categories:
        ...
        if "childs" in nextLvlCategory.keys() and len(nextLvlCategory["childs"]) > 0:
            childCategories, allCategories = next_lvl_category(
                nextLvlCategory["childs"], allCategories
            )
...
async def create_category(
    ...
    parent_category=Query(
        "Не указано", enum=[*[category["title"] for category in db.categories.find()]]
    ),
):
    ...
    db.categories.find_one_and_update(
        {"_id": ObjectId(parentCategory["_id"])}, {"$set": parentCategory}
    )

    return categoryEntity(db.categories.find_one({"_id": childCategory.inserted_id}))
...
async def update_category(
    ...
    parent_category=Query(
        "Не указано",
        enum=[*[cat["title"] for cat in find_all_categories()]]
        if find_all_categories()
        else ["Не указано"],
    ),
):
    category = dict(category)
    ...
    category["childs"] = db.categories.find_one({"_id": ObjectId(category_id)})[
        "childs"
    ]
    ...
    db.categories.find_one_and_update(
        {"_id": ObjectId(newParent["_id"])}, {"$set": newParent}
    )
...
async def delete_category(category_id):
    db.categories.find_one_and_delete({"_id": ObjectId(category_id)})
    db.categories_relation.find_one_and_delete(
        {"child_category_id": ObjectId(category_id)}
    )
    ...
    for relation in categoriesRelation:
        if "parent_category_id" in relation.keys():
            parentCategories.append(
                db.categories.find_one({"_id": relation["parent_category_id"]})
            )
        else:
            parentCategories.append(None)

    return categoriesEntity(db.categories.find().sort("_id"), parentCategories)
